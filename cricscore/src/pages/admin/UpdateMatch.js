...
const UpdateMatch = () => {
  ...
  const getSingleMatch = async () => {
    try {
      const { data } = await axios.get(
        `/api/v1/match/get-match/${params.slug}`
      );
      setName(data.match.name);
      setId(data.match._id);
      setDescription(data.match.description);
      setToss_winner(data.match.toss_winner);
      setTeam_1(data.match.team_1);
      setTeam_2(data.match.team_2);
      setVenue(data.match.venue);
      setTeam_1r(data.match.team_1r);
      setTeam_2r(data.match.team_2r);
      setWinner(data.match.winner);
      setCategory(data.match.category._id);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      photo && matchData.append("photo", photo);
      matchData.append("category", category);

      const { data } = axios.put(
        `/api/v1/match/update-match/${id}`,
        matchData
      );
      if (data?.success) {
              ...
              <div className="mb-3">
                {photo ? (
                  <div className="text-center">
                    ...
                    <div className="text-center">
                    <img
                      src={`/api/v1/match/match-photo/${id}`}
                      alt="match_photo"
                      height={"200px"}
                      className="img img-responsive"
                    />
